...
int	init(int argc, char **argv, t_main_task *main_task)
{
	...
	if (argc == 6)
		main_task->args->number_of_times_each_philosopher_must_eat
			= fn_atoi(argv[5]);
	else
...
int	init_mutex(t_main_task *main_task)
{
	...
	main_task->mutexes->forks = (pthread_mutex_t *)malloc
...
int	init_philosophers(t_main_task *main_task)
{
	...
	main_task->philosophers = (t_philosopher *)malloc
		(sizeof(t_philosopher) * main_task->args->number_of_philosophers);
	...
	while (++i < main_task->args->number_of_philosophers)
	{
		...
		main_task->philosophers[i].right_fork = &main_task->mutexes->forks
		[(i + 1) % main_task->args->number_of_philosophers];
